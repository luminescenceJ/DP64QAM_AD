...
def loss_accuracy_f1_curve(train_loss_history, vali_loss_history, test_loss_history, f1_score_history,
    ...
    if save_path:
        plt.savefig(save_path + "/loss_accuracy_f1_curve.png", dpi=600, bbox_inches='tight', transparent=True)
    plt.show()
...
def confusion_maxtrix_graph(y_true, y_pred, save_path='./graph_result'):
    ...
    for i, label in enumerate(class_labels):
        ...
        ax.text(i + 0.5, len(class_labels) + 0.5, label, ha='center', va='center', fontsize=20,
                fontweight='bold')  # X轴下方的标签
...
def score_my(y_true, y_pred, digits=4, detail=False):
    ...
    weighted_precision = np.sum(
        [report[label]['precision'] * report[label]['support'] for label in labels]) / total_support
    ...
    if detail:
        ...
        for label in labels:
            print(
                f"{label:<15}{report[label]['precision']:<12}{report[label]['recall']:<9}{report[label]['f1-score']:<12}{report[label]['support']:<9}")
        ...
        print(
            f"accuracy:{accuracy:.4f},precision:{macro_precision:.4f},recall:{macro_recall:.4f},f1-score:{macro_f1:.4f}")
...
def distance(pred, true, ratio=[1, 1, 1, 1, -1]):
    pred = np.array(pred)
    true = np.array(true)
    total_euclidean = 0
    total_manhattan = 0
    ...
    total_dtw = 0
    total_cosine = 0
    num_groups = true.shape[0]  # 获取数据组的数量
    for i in range(num_groups):  # `num` 是第一维，表示有多少组数据
        abnormal_input = pred[i].flatten()
        normal_input = true[i].flatten()
    ...
    err = np.array([avg_euclidean, avg_manhattan, avg_chebyshev, avg_dtw, avg_cosine])
    res = np.vdot(ratio, err)
    return res

# pred = np.random.normal(0, 1, 1000)
# true = np.random.normal(0, 1, 1000)
# print(distance(pred,true))
