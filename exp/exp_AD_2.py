...
from utils.tools import EarlyStopping, adjust_learning_rate, loss_accuracy_f1_curve, score_my, distribution_scatter, \
    plot_energy_distribution, visualize
...
class Exp_Anomaly_Detection(Exp_Basic):
    ...
    def train(self, setting):
        ...
        for epoch in range(self.args.train_epochs):
            ...
            vali_loss, vali_label = self.vali(vali_loader, criterion)
            test_loss, test_label = self.vali(test_loader, criterion)
            F1_score = self.F1_vali(vali_loss, vali_label[:, 0], test_loss, test_label[:, 0],
                                    self.args.ratio * train_loss)
    ...
    def test(self, setting, load_model=None):
        ...
        # visualize(vali_data, self.model, batch_size=self.args.batch_size, num=self.args.out_len, itr=2,
        #           save_name='vali')
        # visualize(test_data, self.model, batch_size=self.args.batch_size, num=self.args.out_len, itr=2,
        #           save_name='test')
