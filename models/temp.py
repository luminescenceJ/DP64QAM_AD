...
#         [euclidean(normal_input[i].flatten(), abnormal_input[i].flatten()) for i in range(len(normal_input))])
#
#     # 计算曼哈顿距离
#     manhattan_distance = np.mean(
#         [cityblock(normal_input[i].flatten(), abnormal_input[i].flatten()) for i in range(len(normal_input))])
#
#     # 计算切比雪夫距离
#     chebyshev_distance = np.mean(
#         [chebyshev(normal_input[i].flatten(), abnormal_input[i].flatten()) for i in range(len(normal_input))])
#
#     # 计算动态时间规整（DTW）距离
#     dtw_distance = np.mean([fastdtw(normal_input[i], abnormal_input[i])[0] for i in range(len(normal_input))])
#
#     # 计算余弦相似度
#     cosine_similarity = np.mean(
#         [1 - cosine(normal_input[i].flatten(), abnormal_input[i].flatten()) for i in range(len(normal_input))])
#
#     # 打印结果
...
#
# # 示例数据
# normal_input = np.random.rand(10, 16384, 2)  # 10个正常样本
# abnormal_input = np.random.rand(10, 16384, 2) + 0.5  # 10个异常样本，稍微偏移
#
# # 调用函数
