...
class Model(nn.Module):
    ...
    def __init__(self, args):
        ...
        # computed params
        self.seq_len = math.ceil(args.seq_len / args.modulation) # 1366
        self.seq_ch = args.modulation
        ...
        self.patch_num += 1
        self.enc_embedding = DataEmbedding(self.seq_ch * self.patch_size, args.d_model, args.dropout)

        rwkv_config = RWKVConfig(n_layer=args.n_layers, n_head=args.n_heads, n_embd=args.d_model)
        self.rwkv = nn.ModuleList([Block(rwkv_config, i) for i in range(rwkv_config.n_layer)])
        self.ln_proj = nn.LayerNorm(args.d_ff)
        self.out_layer = nn.Linear(args.d_ff,args.hidden_ch,bias=True)
    ...
    def forward(self, x_enc):
        ...
        seg_num = 32
        padding_needed = seg_num - (x_enc.shape[1] % seg_num)  # 第二维度的填充量
        if padding_needed < seg_num:  # 如果需要填充
            x_enc = F.pad(x_enc, (0, 0, 0, padding_needed))
        ...
        # print(dec_out.shape)  # 打印 dec_out 的形状
        # print(stdev.shape)  # 打印 stdev 的形状
